...
__global__ void mulrow(float *m1, int r1, float *m2, int c2, float *dst,
                       int width) {
  ...
  if (row < r1) {
    for (int col = 0; col < c2; col++) {
      ...
      for (int i = 0; i < width; i++) {
        value += m1[row * width + i] * m2[i * c2 + col];
      }
...
void mulcol(float *m1, int rows1, int cols1, float *m2, int rows2, int cols2,
  ...
  int grid = divCeil(rows1, block_size);
  ...
  mulrow<<<grid, block_size>>>(m1_d, rows1, m2_d, cols2, dst_d, cols1);
