...
__global__ void tiled_matmul(float *m1, int r1, int c1, float *m2_t, int c2,
                             float *dst) {
  ...
  for (int i = 0; i < number_tiles; ++i) {
    ...
    // swapped threadIdx.x and threadIdx.y
    tileM2[threadIdx.x][threadIdx.y] =
        (col < c2 && tile_row < c1) ? m2_t[tile_row * c2 + col] : 0.0f;
...
void mul(float *m1_h, int r1, int c1, float *m2_t_h, int c2, float *dst_h) {
  ...
  cudaMemcpy(m2_d, m2_t_h, sizeM2, cudaMemcpyHostToDevice);
...
float *transpose(float *m, int rows, int cols) {
  float *m2 = (float *)malloc(rows * cols * sizeof(float));
  int size = rows * cols;
...
int main() {
  ...
  float *m2_t = transpose(m2, rows2, cols2);
  float *dst = (float *)malloc(rows1 * cols2 * sizeof(float));
  float *dst2 = (float *)malloc(rows1 * cols2 * sizeof(float));

  for (int i = 0; i < rows1 * cols2; i++) {
    dst[i] = 0.0f;
  }

  mul(m1, rows1, cols1, m2_t, rows2, dst);
//   mul_cpu(m1, rows1, cols1, m2, cols2, dst2);

//   printm(m2, rows2, cols2);
//   printf("\n");
//   printm(m2_t, cols2, rows2);
//   printf("\n");

//   printm(dst, rows1, cols1);
//   printf("\n");
//   printm(dst2, rows2, cols2);
